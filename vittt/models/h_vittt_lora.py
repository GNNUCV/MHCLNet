...
class LoRAQKVLinear(nn.Module):
    """LoRA wrapper for the fused qkv projection in ViT^3 TTT attention.

    This follows the original LoRA idea: keep the pretrained projection frozen
    and learn a low-rank residual update Delta W = B A with scaling alpha / r.
    The update is applied only to query and value parts. The key parts and the
    original projection weights are unchanged.
    """

    def __init__(self, base_linear, dim, num_heads, r=8, lora_alpha=16, lora_dropout=0.0):
        super().__init__()
        if not isinstance(base_linear, nn.Linear):
            raise TypeError("LoRAQKVLinear expects an nn.Linear module as base_linear.")
        if dim % num_heads != 0:
            raise ValueError(f"dim={dim} must be divisible by num_heads={num_heads}.")

        head_dim = dim // num_heads
        expected_out_features = dim * 3 + head_dim * 3
        if base_linear.in_features != dim or base_linear.out_features != expected_out_features:
            raise ValueError(
                "The wrapped qkv projection shape does not match ViT^3 TTT attention: "
                f"got in={base_linear.in_features}, out={base_linear.out_features}, "
                f"expected in={dim}, out={expected_out_features}."
            )
        ...
        # Original LoRA freezes the pretrained weight matrix and trains only A and B.
        for param in self.base.parameters():
            param.requires_grad = False

        if r > 0:
            # Branch 1 query/value: q1 and v1 have output dimension dim.
            self.lora_q1_A = nn.Parameter(torch.empty(r, dim))
            self.lora_q1_B = nn.Parameter(torch.zeros(dim, r))
            self.lora_v1_A = nn.Parameter(torch.empty(r, dim))
            self.lora_v1_B = nn.Parameter(torch.zeros(dim, r))

            # Branch 2 query/value: q2 and v2 have output dimension head_dim.
            self.lora_q2_A = nn.Parameter(torch.empty(r, dim))
            self.lora_q2_B = nn.Parameter(torch.zeros(head_dim, r))
            self.lora_v2_A = nn.Parameter(torch.empty(r, dim))
            self.lora_v2_B = nn.Parameter(torch.zeros(head_dim, r))

            self.reset_lora_parameters()

    def reset_lora_parameters(self):
        # LoRA initialization: A uses a standard random initialization, B is zero.
        # Therefore the LoRA branch initially contributes exactly zero.
        for name, param in self.named_parameters():
            if "lora_" in name and name.endswith("_A"):
                nn.init.kaiming_uniform_(param, a=5 ** 0.5)
            elif "lora_" in name and name.endswith("_B"):
                nn.init.zeros_(param)

    def _lora_update(self, x, lora_A, lora_B):
        x = self.lora_dropout(x)
        return F.linear(F.linear(x, lora_A), lora_B) * self.scaling
    ...
    def forward(self, x):
        ...
        c = self.dim
        d = self.head_dim
        q1, k1, v1, q2, k2, v2 = torch.split(out, [c, c, c, d, d, d], dim=-1)

        q1 = q1 + self._lora_update(x, self.lora_q1_A, self.lora_q1_B)
        v1 = v1 + self._lora_update(x, self.lora_v1_A, self.lora_v1_B)
        q2 = q2 + self._lora_update(x, self.lora_q2_A, self.lora_q2_B)
        v2 = v2 + self._lora_update(x, self.lora_v2_A, self.lora_v2_B)

        return torch.cat([q1, k1, v1, q2, k2, v2], dim=-1)


def add_lora_to_ttt_attention(attn, dim, num_heads, r=8, lora_alpha=16, lora_dropout=0.0):
    """Attach LoRA to the fused qkv projection of a TTT attention block."""
...
class TTTBlock(nn.Module):
    ...
    def __init__(self, dim, input_resolution, num_heads, mlp_ratio=4., qkv_bias=True, drop=0., drop_path=0.,
        ...
        self.attn = add_lora_to_ttt_attention(
            self.attn,
            dim=dim,
            num_heads=num_heads,
            r=8,
