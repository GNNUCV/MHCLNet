...
from .AdaptFormer import AdapterCfg,AdaptFormer
...
class TTTBlock(nn.Module):
    ...
    def __init__(self, dim, input_resolution, num_heads, mlp_ratio=4., qkv_bias=True, drop=0., drop_path=0.,
        ...
        self.conf = AdapterCfg(embed_dims=dim,inner_dim=64)
        self.af1 = AdaptFormer(self.conf)
        self.af2 = AdaptFormer(self.conf)
    ...
    def forward(self, x):
        ...
        x = self.af1(x,hw_shape)

        # FFN
        x = x + self.drop_path(self.mlp(self.norm2(x), H, W))

        x = self.af2(x, hw_shape)
