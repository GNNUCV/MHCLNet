...
def parse_args():
    ...
    parser.add_argument(
        '--save-input',
        type=Path,
        help='Path to save the 224x224 input image before preprocessing.')
...
def main():
    ...
    # data['inputs'] 是已经随机裁剪 / resize 后的 224x224，形状 [C, H, W]
    # 先备份为 HWC 格式（BGR），既用于后面的 CAM 融合，也可选择保存下来
    src_img = copy.deepcopy(data['inputs']).numpy().transpose(1, 2, 0)

    # 如果指定了 --save-input，就把这张 224x224 输入图像保存到硬盘
    if args.save_input is not None:
        # 转成 uint8 再写入文件
        mmcv.imwrite(src_img.astype(np.uint8), str(args.save_input))
        print(f'Saved 224x224 input image to: {args.save_input}')

    # 再交给 data_preprocessor 做归一化等预处理
    ...
    cam_grad = init_cam(args.method, model, target_layers, use_cuda,
