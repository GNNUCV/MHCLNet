...
# ----------------------------
# Official LoRA (Low-Rank Adaptation) minimal implementation
# Paper: "LoRA: Low-Rank Adaptation of Large Language Models" (Hu et al., 2021)
# This implementation follows the common "loralib" behavior:
#   y = W x + (alpha / r) * B(A(dropout(x)))
# Base weight W can be frozen for parameter-efficient finetuning.
# ----------------------------
import math
...
class LoRALinear(nn.Linear):
    ...
    Args:
        ...
        r (int): LoRA rank. r=0 disables LoRA.
        lora_alpha (float): scaling alpha. Effective scale = alpha / r.
        lora_dropout (float): dropout on the LoRA branch input.
        merge_weights (bool): if True, allow merging LoRA weights into base
            weight for inference.
        freeze_base (bool): if True, freeze the base linear weight/bias.
    """
    def __init__(
        self,
        in_features: int,
        ...
        if self.r > 0:
            ...
            # Init: A Kaiming, B zeros => start from the pretrained function
    ...
    def forward(self, x):
        ...
        if self.r > 0 and (not self.merged):
            ...
            # (..., in) -> (..., r) -> (..., out)
            result = result + F.linear(F.linear(x_d, self.lora_A), self.lora_B) * self.scaling
        return result


def mark_only_lora_trainable(module: nn.Module, train_bias: bool = False):
    """Freeze everything except LoRA parameters (and optionally biases)."""
...
# ----------------------------
# LoRA injection utilities for SwinTransformer (qkv/proj by default)
# ----------------------------

def _build_lora_linear_from_linear(
    linear: nn.Linear,
    r: int,
    lora_alpha: float,
    lora_dropout: float,
    ...
    """Create a LoRALinear from an existing nn.Linear and copy weights."""
    new = LoRALinear(
        linear.in_features,
        linear.out_features,
...
def _inject_lora_into_module(
    ...
    """Recursively replace selected nn.Linear with LoRALinear inside `module`.

    Args:
        module: root module to modify.
        target_names: attribute names to match. If None or contains '*', inject
            into all nn.Linear inside this module.
    Returns:
    ...
    if target_names is None:
        ...
        match_all = ('*' in name_set)

    replaced = 0
    for child_name, child in module.named_children():
        # Replace at this level
        if isinstance(child, nn.Linear) and (match_all or name_set is None or child_name in name_set):
            if isinstance(child, LoRALinear):
                continue
            new_child = _build_lora_linear_from_linear(
                child,
                r=r,
                lora_alpha=lora_alpha,
                lora_dropout=lora_dropout,
            ...
            replaced += _inject_lora_into_module(
                child,
...
def inject_lora_into_swin(
    ...
    attn_linear_names: Sequence[str] = ('qkv', 'proj'),
    ffn_linear_names: Sequence[str] = ('fc1', 'fc2'),
    merge_weights: bool = False,
    freeze_base: bool = True,
) -> int:
    """Inject LoRA into SwinTransformer blocks.

    By default, injects into attention qkv/proj linears in each SwinBlock.
    Optionally injects into FFN linears too.

    Returns:
        Total number of nn.Linear replaced by LoRALinear.
    """
    total = 0
    for m in model.modules():
        # `SwinBlock` is defined in this file (same class used to build stages)
        if isinstance(m, SwinBlock):
            if enable_attn and hasattr(m, 'attn'):
                total += _inject_lora_into_module(
                    m.attn,
                    target_names=attn_linear_names,
                    r=r,
                    lora_alpha=lora_alpha,
                    lora_dropout=lora_dropout,
                    merge_weights=merge_weights,
            ...
            if enable_ffn and hasattr(m, 'ffn'):
                total += _inject_lora_into_module(
                    m.ffn,
                    target_names=ffn_linear_names,
                    r=r,
                    lora_alpha=lora_alpha,
                    lora_dropout=lora_dropout,
                    merge_weights=merge_weights,
    ...
    # mark for freeze logic
    if total > 0:
        setattr(model, '_has_lora', True)
    return total
...
class SwinTransformer(BaseBackbone):
    ...
    def __init__(self,
        ...
        # Example:
        #   lora_cfg=dict(r=8, lora_alpha=16, lora_dropout=0.05,
        #                enable_attn=True, enable_ffn=False,
        #                attn_linear_names=('qkv','proj'),
        #                freeze_base=True, only_lora_trainable=True)
        self.lora_cfg = lora_cfg
        if lora_cfg is not None:
            assert isinstance(lora_cfg, dict), 'lora_cfg must be a dict or None'
            _lcfg = dict(
                r=lora_cfg.get('r', 8),
                lora_alpha=lora_cfg.get('lora_alpha', lora_cfg.get('alpha', 8.0)),
                lora_dropout=lora_cfg.get('lora_dropout', lora_cfg.get('dropout', 0.0)),
                enable_attn=lora_cfg.get('enable_attn', True),
                enable_ffn=lora_cfg.get('enable_ffn', False),
                attn_linear_names=tuple(lora_cfg.get('attn_linear_names', ('qkv', 'proj'))),
                merge_weights=lora_cfg.get('merge_weights', False),
                freeze_base=lora_cfg.get('freeze_base', True),
            )
            inject_lora_into_swin(self, **_lcfg)

            if lora_cfg.get('only_lora_trainable', True):
                mark_only_lora_trainable(
                    self, train_bias=bool(lora_cfg.get('train_bias', False)))
    ...
    # ---- LoRA helpers ----
    def merge_lora(self):
        """Merge all LoRA weights into base weights (useful for inference)."""
        for m in self.modules():
            if isinstance(m, LoRALinear):
                m.merge()

    def unmerge_lora(self):
        """Undo merge of LoRA weights."""
        for m in self.modules():
            if isinstance(m, LoRALinear):
                m.unmerge()
